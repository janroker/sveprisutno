...
void* chip_init() {
  ...
  printf("Hello from custom ALU chip!\n");
...
uint8_t on_i2c_read(void *user_ctx) {
  ...
  if (chip->operation == OPERATION_MUL){
      rez.f = chip->par1.f * chip->par2.f;
  }else if (chip->operation == OPERATION_DIV){
    if(chip->par2.f == 0.0 ) 
      rez.f = 0.0;
    else 
      rez.f = chip->par1.f / chip->par2.f;
  }else if (chip->operation == OPERATION_ADD){
      rez.f = chip->par1.f + chip->par2.f;
  }else if (chip->operation == OPERATION_SUB){
      rez.f = chip->par1.f - chip->par2.f;
  }else if (chip->operation == OPERATION_POWER_A){
      rez.f = chip->par1.f *chip->par1.f;
  }else if (chip->operation == OPERATION_POWER_B){
      rez.f = chip->par2.f *chip->par2.f;
  }else if (chip->operation == OPERATION_SQRT_A){
      rez.f = sqrtf(chip->par1.f);
  }else if (chip->operation == OPERATION_SQRT_B){
      rez.f = sqrtf(chip->par2.f);
  }else if (chip->operation == OPERATION_READ_A){
      rez.f = chip->par1.f;
  }else if (chip->operation == OPERATION_READ_B){
      rez.f = chip->par2.f;
  }else{
    rez.f = -0.1010101;
  }
