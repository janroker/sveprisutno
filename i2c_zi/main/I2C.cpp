...
esp_err_t I2C::writeBytes(uint8_t reg_addr, uint8_t *data, uint8_t device_address, uint32_t i2c_timeout_ms, int num_data)
{
    uint8_t write_buf[num_data + 1];
...
esp_err_t I2C::deviceRead(uint8_t *data, size_t len, uint8_t device_address, uint32_t i2c_timeout_ms)
{
    return i2c_master_read_from_device(this->i2cNum, device_address, data, len, i2c_timeout_ms / portTICK_PERIOD_MS);
}
